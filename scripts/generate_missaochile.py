#!/usr/bin/env python3
"""
generate_missaochile.py
Busca dados do board monday.com 18420887942 (Missão Comercial Chile)
e gera index.html no repositório GitHub configurado.
          ...
          boards(ids: [{BOARD_ID}]) {{
            items_page(limit: 100{cursor_arg}) {{
              ...
              items {{
                id name
                column_values(ids: [
                  "{COL_EMPRESA}", "{COL_RESP}", "{COL_FUNCAO}", "{COL_PAISES}",
...
def build_html(items_raw):
    ...
    for d in items:
        ...
        if raw:
            for p in _re.split(r'[\s,;/\n]+', raw):
                ...
                if len(p) > 1:
                    _pfreq[p.title()] += 1
    paises_data = [[w, max(cnt * 14, 16)] for w, cnt in _pfreq.most_common(40)]
    ...
    sat_html = (
        ...
        sat_bar("Atendimento BTCoffeeNation", sat_atd) +
...
<title>Relatório de Avaliação – Missão Comercial Chile 2026 | BSCA</title>
...
#paisesCloudCanvas {{ width:100%!important; height:200px!important; }}
/* ── Qualitative cards ──────────────────────────────────── */
.qual-grid {{
...
<section class="hero">
  <div class="hero-inner">
    ...
    <h1>Relatório de Avaliação<br><span>Missão Comercial Chile 2026</span></h1>
    <p>Resultados consolidados das avaliações recebidas das empresas participantes da Missão Comercial Chile 2026.</p>
  </div>
</section>
  ...
  <div class="chart-row cols-21" style="margin-bottom:14px">
    <div class="card">
      ...
      <div style="margin-top:16px">
        <div class="card-title" style="margin-bottom:8px">Países dos contatos — nuvem de palavras</div>
        <canvas id="paisesCloudCanvas" height="200"></canvas>
      </div>
      <div style="margin-top:16px">
...
<footer class="site-footer">
  ...
  <p>BSCA – Associação Brasileira de Cafés Especiais &nbsp;·&nbsp; Projeto Brazil. The Coffee Nation &nbsp;·&nbsp; Missão Comercial Chile 2026</p>
</footer>
...
new Chart(document.getElementById('chartContacts'), {{
  ...
  options: {{
    ...
    scales:{{
      x:{{ grid:{{ color:'#EDE6D8' }}, ticks:{{ stepSize:1 }} }},
...
// ── Word cloud (Países) ────────────────────────────────────────────────────
const paisesData = {json.dumps(paises_data, ensure_ascii=False)};
if (paisesData.length > 0 && typeof WordCloud !== 'undefined') {{
  const pcCanvas = document.getElementById('paisesCloudCanvas');
  pcCanvas.width  = pcCanvas.parentElement.offsetWidth || 500;
  pcCanvas.height = 200;
  WordCloud(pcCanvas, {{
    list: paisesData,
    gridSize: 8,
    ...
    fontFamily: "'Inter', sans-serif",
    color: () => {{
      const palette = ['#2D5E42','#4A8C64','#7D5A35','#C9A84C','#4A7FA5','#1E3D2F'];
      return palette[Math.floor(Math.random() * palette.length)];
    }},
    rotateRatio: 0.2,
    rotationSteps: 2,
  ...
  const el = document.getElementById('paisesCloudCanvas');
  el.style.display = 'none';
  el.insertAdjacentHTML('afterend','<p style="font-size:12px;color:#aaa;padding:8px 0">Nenhuma resposta registrada.</p>');
}}
</script>
...
def push_to_github(html_content):
    ...
    GH_REPO   = os.environ.get("GITHUB_REPOSITORY", "renanfreitas-sketch/missaochile2026")
    ...
    payload = {
        "message": f"missaochile: auto-update {now}",
...
if __name__ == "__main__":
    ...
    print("Fetching monday.com data (Missão Comercial Chile 2026)…")
    raw = fetch_items()
    ...
    if "--dry-run" in sys.argv:
        with open("missaochile2026.html", "w", encoding="utf-8") as f:
            f.write(html)
        print("Saved to missaochile2026.html (dry run)")
    else:
        push_to_github(html)
        print("Done.")
